...
| 模块 | 说明 |
|------|------|
| 仪表盘 Dashboard | KPI：今日会话、自动解决率、待人工、店铺状态；折线/柱状占位 |
| 店铺绑定 | Shopee OAuth 入口、连接状态、Token 过期提示 |
| 收件箱 / 会话 | 会话列表、消息时间线、转人工标记 |
| AI 设置 | 语气、自动回复开关、业务时段、知识片段（简版） |
| 计费 Billing | 套餐、用量、发票入口（可先静态 + API 桩） |

**产出**：H5/桌面宽屏可用的商家端主路径，中文文案统一。
...
3. 商家端 Dashboard + 店铺绑定页视觉重做（下一迭代）
