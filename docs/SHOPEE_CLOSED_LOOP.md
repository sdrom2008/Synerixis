...
| Intent ↔ Agent | ⚠️ | `OrderAgent.SupportedIntent = QueryOrder` vs Classifier 映射 `OrderQuery` | 枚举名不一致，路由易落空 |
| 订单查询（DB） | ✅ | `OrderAgent` → `IOrderRepository` | B2C 按 shop+customer 查库 |
| 订单查询（平台 API） | ✅（客户端） / ❌（未接入 Agent） | `ShopeePlatformClient.GetCustomerOrderAsync` | 已实现 v2 `get_order_detail` / `get_order_list`；**OrderAgent 未注入 `IPlatformClient`** |
...
- ⚠️ `QueryOrder` / `OrderQuery` 命名分裂  

### 4. Order → Reply

- ✅ DB 路径：`OrderAgent`  
- ✅ 平台路径：`GetCustomerOrderAsync`（order_sn 含字母 → detail；否则近期 list + detail 匹配 `buyer_user_id`；返回 `order_sn/status/tracking` 摘要）  
- ❌ **下一刀接线**：当 `IOrderRepository` 无数据时，OrderAgent fallback 调用平台 `GetCustomerOrderAsync`（当前故意不大改 Agent，避免范围膨胀）
